        ...
        if not purchases:
            ...
            for purchase in purchases:
                if  purchase['symbol'] == symbol:
                    new_shares = int  (purchase['shares']) + int(shares)
            db.execute("UPDATE purchases SET shares= ? WHERE purchases.user_id=? AND purchases.symbol = ? ",new_shares,user_id,symbol)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if int(shares) <= 0 or  not symbol:
             return apology("Must provide a valid value.",400)
        current_shares =db.execute("SELECT * FROM purchases WHERE purchases.user_id = ? and purchases.symbol= ? " ,user_id,symbol)[0]
        if int(current_shares['shares']) < int(shares):
            return apology("TOO MANY SHARES",400)
        new_shares =  int(current_shares['shares']) - int(shares)
        quote=lookup(symbol)
        total_price = int(shares) * int(quote['price'])
        if int(new_shares) <= 0:
            db.execute("DELETE FROM purchases WHERE purchases.user_id = ? AND purchases.symbol= ? ",user_id,symbol)
        else:
            db.execute("UPDATE purchases SET shares = ?",new_shares)
        current_cash = db.execute("SELECT users.cash FROM users")[0]

        new_cash = total_price + int(current_cash['cash'])

        db.execute("UPDATE purchases SET cash = ? ",new_cash)
